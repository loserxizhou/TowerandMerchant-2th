...
import Proto.LoginRequestProto;
...
public class UserController extends BaseController {
    ...
    public String Login(String jsonData,Channel channel,Server server)
    {
        ...
        String email=loginRequestProto.getEmails();
...
//            {
//                //todo 暂时不处理设置名字，如果没有名字则是用email作为
//                //说明角色名字为空，没有创建
//                //发生一个响应，去让客户端创建游戏昵称
...
//            }
            ...
            //LoginResponseProto loginResponseProto=new LoginResponseProto();
            if(userSave.getNickname().equals("") ||userSave.getNickname()==null)
            {
                //没有nickname时使用email//int coins,int diamonds,int userid,String email,int roleid,int exp
                LoginResponseProto loginSuccessfulResponseProto=new LoginResponseProto(ReturnType.Successful,user.getEmail(),userSave.getBaselevel(),userSave.getCoin(),userSave.getDiamond(),userSave.getUserid(),user.getEmail(),userSave.getRoleid(),userSave.getExp());
                String responseJsonData=LoginResponseProto.packJsonData(loginSuccessfulResponseProto,true);
                return responseJsonData;
            }else{
                //有nickname
                LoginResponseProto loginSuccessfulResponseProto=new LoginResponseProto(ReturnType.Successful,userSave.getNickname(),userSave.getBaselevel(),userSave.getCoin(),userSave.getDiamond(),userSave.getUserid(),user.getEmail(),userSave.getRoleid(),userSave.getExp());
                String responseJsonData=LoginResponseProto.packJsonData(loginSuccessfulResponseProto,true);
                return responseJsonData;
            }
